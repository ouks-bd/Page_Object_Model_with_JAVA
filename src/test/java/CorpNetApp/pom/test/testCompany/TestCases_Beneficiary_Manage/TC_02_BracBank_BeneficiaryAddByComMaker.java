...
public class TC_02_BracBank_BeneficiaryAddByComMaker extends BaseTest {

    Add_Beneficiary_Page _add_beneficiary_page;

    public TC_02_BracBank_BeneficiaryAddByComMaker(){
        super();
    }
    @BeforeMethod
    public void setUp(){
        initialization();
        _add_beneficiary_page =new CompanyUsersLoginPage().CompMCUserLogin(getTranMCUser(),getTranMakerPass()).clickBRACBankBeneficiaryMenu();
    }
    @Test
    public void addPayrollBeneficiarySuccess() throws InterruptedException {
        _add_beneficiary_page = _add_beneficiary_page
                .fillBRACBankBeneficiaryInfo()
                .clickBtn();
